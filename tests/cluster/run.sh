...
# Each entry is single-quoted here because the whole command is composed into ONE ssh -c string
# below - an unquoted value with spaces (e.g. a POLICY_FILE path) would word-split there.
FWD=()
for v in USE_RELEASED NAMESPACE TRACEE_IMAGE POLICY_FILE READY_TIMEOUT KEEP K3S_VERSION HELM_VERSION; do
    [ -n "${!v:-}" ] && FWD+=("${v}='${!v}'")
done
