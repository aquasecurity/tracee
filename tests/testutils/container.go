...
// ProcessRunning reports whether a process with the given comm name is
// running, by scanning /proc directly: no dependency on pgrep/procps, which
// not every build environment ships, and works identically on the host and
// in --pid=host containers.
