...
# run CO-RE FILE_MODIFICATION test only by default
TESTS=${INSTTESTS:=FILE_MODIFICATION}
...
set -e
make -j$(nproc) all
make e2e-instrumentation-rules
set +e
...
for TEST in $TESTS; do
    ...
    events=$(./dist/tracee-rules --allcaps --rules-dir ./dist/e2e-instrumentation-rules/ --rules $TEST --list-events)
    ...
    ./dist/tracee-ebpf \
        ...
        | \
    ./dist/tracee-rules \
        --rules-dir ./dist/e2e-instrumentation-rules/ \
    ...
    timeout --preserve-status 20 ./tests/e2e-instrumentation-rules/scripts/${TEST,,}.sh
