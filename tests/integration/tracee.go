...
func (b *eventBuffer) addEvent(evt pipeline.Event) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *eventBuffer) clear() {
	...
	b.events = make([]pipeline.Event, 0)
}
...
func (b *eventBuffer) getCopy() []pipeline.Event {
	b.mu.RLock()
	defer b.mu.RUnlock()

	evts := make([]pipeline.Event, len(b.events))
	copy(evts, b.events)
