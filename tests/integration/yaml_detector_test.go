...
func startTraceeWithYAMLDetectors(ctx context.Context, t *testing.T, yamlDir string) (*tracee.Tracee, *testutils.EventBuffer) {
...
func waitForDetectorEvent(buf *testutils.EventBuffer, eventName string, timeout time.Duration) *trace.Event {
...
// getArgValue extracts an argument value from an event by name
func getArgValue(evt *trace.Event, argName string) interface{} {
	arg, err := evt.GetArgumentByName(argName, trace.GetArgOps{DefaultArgs: false})
	if err != nil {
		return nil
	}
	return arg.Value
...
output:
  extract_fields:
    - name: binary_path
      source: data.pathname
    - name: binary_name
      source: workload.process.thread.name
    - name: pid
...
`
	...
	trc, buf := startTraceeWithYAMLDetectors(ctx, t, yamlDir)
	defer func() {
		cancel()
	...
	t.Logf("Found event: %s", evt.EventName)
...
`
	...
	t.Logf("Found level 1 event: %s", level1Evt.EventName)
	...
	t.Logf("Found level 2 event: %s", level2Evt.EventName)
...
`
	...
	assert.Nil(t, unmatchedEvt, "Detector should NOT fire for non-matching pathname /bin/date")
	t.Logf("✓ Filter correctly rejected /bin/date")
}
