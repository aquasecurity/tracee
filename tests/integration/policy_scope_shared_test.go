...
// harness (withPolicy/withPolicies): each case applies its policies at runtime, runs its workload, asserts
// attribution, then removes the policies. This replaces the per-test tracee start/stop (~6-8s each) for this
...
//
// WHAT THE EXIT-BASED CASES PROVE (and what they cannot): the internal bootstrap policy always collects
// sched_process_exec/fork/exit UNFILTERED, so every exit - matching or not - is submitted to userland and
// stats.EventsFiltered can never move for these events (the bootstrap rule bit keeps the bitmap non-empty;
// the complement is discarded at the SINK, with no user-selected match). EventsFiltered is therefore NOT a
// kernel-vs-userland signal here; asserting it would be vacuously green. What IS falsifiable: a policy-level
// scope (comm/uid equality/...) has NO userland re-evaluation - the kernel filter is the only thing that can
// clear a policy's rule bit. So "complement emitted to zero policies" proves the KERNEL cleared the bit
// (attribution narrowing), and mis-attribution on any emitted event proves it didn't. True submission gating
// (EventsFiltered as a kernel signal) is exercised on a NON-bootstrap event by the per-rule openat case below
// and by Test_PolicyPerRuleBinaryScopeKernelPushdown.
func Test_PolicyScopeShared(t *testing.T) {
	...
	// to pol-a; the complement must be emitted to NO policy - and since a policy-level comm scope has no
	// userland re-evaluation, only the kernel filter can have cleared pol-a's bit on it.
	t.Run("single kernel pushdown", func(t *testing.T) {
		...
		withPolicy(t, trc, buf, exitScopePolicy(10, "pol-a", commA), func(t *testing.T) {
			...
			require.Zero(t, noneCount,
				"complement exits emitted: the kernel did not clear pol-a's rule bit for comm!=%s "+
					"(policy-level comm scope has no userland backstop)", commA)
		})
	})

	// A4 (was Test_PolicyScopeUnionKernelPushdown): two policies scoping DISTINCT comms. The kernel filter is
	// the union {A,B}: A-exits attributed to pol-a only, B-exits to pol-b only (each policy's bit cleared in
	// the kernel for the other's comm), complement emitted to neither.
	t.Run("union kernel pushdown", func(t *testing.T) {
		...
		withPolicies(t, trc, buf, pfs, func(t *testing.T) {
			...
			_, noneCount := exitPoliciesForComm(buf, commNone)
			require.Zero(t, noneCount,
				"complement (matches neither scope) emitted: the union kernel filter {%s,%s} failed to clear both bits", commA, commB)
		})
	})
	...
	t.Run("union defeat", func(t *testing.T) {
		...
		withPolicies(t, trc, buf, pfs, func(t *testing.T) {
			...
			// The complement matching ONLY pol-broad is the falsifiable core: if pol-a's kernel comm filter
			// were not enforced, its bit would survive and the complement would carry pol-a too.
	...
	t.Run("identical union attribution", func(t *testing.T) {
		...
		withPolicies(t, trc, buf, pfs, func(t *testing.T) {
			...
			require.Zero(t, none, "complement exits emitted: the kernel failed to clear both identical-scope bits")
		})
	})
	...
	t.Run("cross dimension union", func(t *testing.T) {
		...
		withPolicies(t, trc, buf, pfs, func(t *testing.T) {
			...
			require.Zero(t, none,
				"exit matching neither dimension emitted: the OR union {comm=A, uid=U} failed to clear both bits in the kernel")
		})
	})
	...
	t.Run("matched subset attribution", func(t *testing.T) {
		...
		withPolicies(t, trc, buf, pfs, func(t *testing.T) {
			...
			// Perf-buffer delivery is per-CPU and not globally ordered, so waiting on Z alone does not
			// imply X/Y arrived - wait on each comm before reading its attribution.
			require.GreaterOrEqual(t, waitForExitComm(buf, commX, 15, 10*time.Second), 15)
			require.GreaterOrEqual(t, waitForExitComm(buf, commY, 15, 10*time.Second), 15)
			...
			require.Zero(t, none, "complement emitted: the kernel failed to clear all five scope bits")
		})
	})
	...
	//
	// Unlike the exit-based cases, openat has no bootstrap rule, so EventsFiltered here IS a genuine
	// kernel-vs-userland discriminator: a per-rule filter is userland-backstopped (matchPolicies re-evaluates
	// it), so if the kernel failed to gate submission, every complement openat would reach userland, be
	// cleared there, empty the bitmap and bump the counter.
	t.Run("per-rule openat kernel pushdown", func(t *testing.T) {
		...
		withPolicy(t, trc, buf, openatPerRuleCommPolicy(40, "perrule", commA), func(t *testing.T) {
			...
			require.Zero(t, emittedCountByComm(buf, "openat", commNone),
				"complement openat events emitted: the per-rule comm scope was not enforced anywhere")
