...
//
// Base events and what each can prove:
//   - sched_process_exit/exec are ALWAYS collected by the internal bootstrap policy (unfiltered), so they
//     are always submitted and EventsFiltered can never move for them - it is NOT a kernel signal there.
//     What is falsifiable on them: policy-level scopes have no userland re-evaluation, so complement
//     emission/attribution can only be narrowed by the KERNEL clearing rule bits.
//   - openat has no bootstrap rule, so EventsFiltered IS a valid kernel-vs-userland signal for
//     userland-backstopped filters (per-rule scope), and complement-absence is the signal for kernel-only
//     filters (executable/binary, which userland never re-evaluates).
...
func Test_PolicyPerRuleScopeDerivedEvent(t *testing.T) {
	...
	policies := testutils.NewPolicies([]testutils.PolicyFileWithID{
		...
		{
			// POLICY-LEVEL scope on a net event. Unlike the per-rule filters above (which have a
			// userland backstop in matchPolicies), a policy-level scope on net events is enforced
			// ONLY by the userland re-run of the kernel scope formula (the socket-bound kernel
			// bitmap is discarded at decode) - so this is the falsifiable regression guard for
			// that pass: without it, this policy would match EVERY ICMP packet on the host.
			Id: 3,
			PolicyFile: polv1beta1.PolicyFile{
				Metadata: polv1beta1.Metadata{Name: "icmp-policy-scope"},
				Spec: k8s.PolicySpec{
					Scope:          []string{"comm=zzznotping"},
					DefaultActions: []string{"log"},
					Rules:          []k8s.Rule{{Event: "net_packet_icmp"}},
				},
			},
		},
	})
	...
	// The non-matching POLICY-LEVEL scope must not match either: policy scope for net events is
	// enforced solely by the userland kernel-formula re-run, so a regression there makes this
	// policy match every ICMP packet on the host.
	require.False(t, hasMatchedEvent(buf, "net_packet_icmp", "ping", "icmp-policy-scope"),
		"net_packet_icmp from ping must NOT match the policy-level scope comm=zzznotping (userland scope re-run regressed)")
}
...
// submission, while openats from the scoped binary flow.
//
// The falsifiable signal is complement ABSENCE, not EventsFiltered: userland never re-evaluates binary
// filters (ScopeFilter.Filter has no binary check), so a kernel miss would emit the complement fully
// attributed - it could never be "filtered in userland" and bump the counter instead.
func Test_PolicyPerRuleBinaryScopeKernelPushdown(t *testing.T) {
	...
	// Kernel enforcement, the assertion that can actually fail: with no userland binary re-evaluation,
	// any openat from the non-scoped binary that the kernel submitted would keep its rule bit and be
	// EMITTED. Zero emission therefore proves the kernel dropped the complement before submission.
	require.Zero(t, emittedCountByComm(buf, "openat", commNone),
		"openat from the non-scoped binary was emitted: the per-rule executable scope was not kernel-enforced "+
			"(and userland has no binary backstop to catch it)")
}
