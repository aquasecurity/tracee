...
func expectEvent(
	...
	return trace.Event{
		...
		HostName:             host,
		EventID:              int(eventID),
		MatchedPolicies:      matchPols,
		MatchedPoliciesNames: matchPolName,
		Args:                 args,
	}
}
...
func ExpectAtLeastOneOfEach(t *testing.T, cmdEvents []cmdEvents, actual *eventBuffer, useSyscaller bool) {
	for _, exp := range cmdEvents {
		...
		for _, expEvt := range exp.evts {
			...
			checkPolicy := expEvt.MatchedPolicies != anyPolicy
			checkPolicyName := len(expEvt.MatchedPoliciesNames) > 0 && expEvt.MatchedPoliciesNames[0] != anyPolicyName
			...
			for _, actEvt := range actEvtsCopy {
				...
				if checkPolicy && actEvt.MatchedPolicies != expEvt.MatchedPolicies {
					continue
				}
				if checkPolicyName {
					polNameFound := false
					for _, policyName := range expEvt.MatchedPoliciesNames {
						for _, actPolicyName := range actEvt.MatchedPoliciesNames {
	...
	for _, exp := range cmdEvents {
		...
		for _, expEvt := range exp.evts {
			...
			for _, actEvt := range actEvtsCopy {
				...
				if checkPolicyName {
					assertUnorderedStringSlicesEqual(t, expEvt.MatchedPoliciesNames, actEvt.MatchedPoliciesNames)
				}
