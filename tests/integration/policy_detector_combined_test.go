...
// scopeExitDetectorYAML is a detector requiring sched_process_exit scoped to a comm. Format args: id,
// produced-event name, comm. Exit fires exactly once per process, so these combined scenarios can assert
// EXACT detector counts (once per matching process). Note what exit CANNOT prove: it is always collected
// by the internal bootstrap policy, so it is always submitted and stats.EventsFiltered never moves for it -
// kernel-gating claims need a non-bootstrap base (see scopeOpenatDetectorYAML). The assertions here are
// behavioral (attribution + emission), which stay falsifiable on exit because a policy-level comm scope has
// no userland re-evaluation.
const scopeExitDetectorYAML = `type: detector
id: %s
produced_event:
  name: %s
  version: 1.0.0
  description: Combined policy+detector scenario test detector
  tags:
    - test
  fields:
    - name: pid
...
// detector's kernel comm filter - every exit keeps at least the broad rule's bit - yet the detector stays
// correct, firing only for commX because the dispatcher applies each detector's own scope filter
// (dispatch.go).
//
// The proof is behavioral (S5): the COMPLEMENT base events now appear in the stream attributed to the
// broad policy (in the scoped-only tests they are emitted to no policy and discarded at the sink), while
// the detector output fires exactly once per commX exit and never for the complement.
func Test_PolicyBroadDefeatsDetectorScope(t *testing.T) {
	...
	// The broad policy defeated the kernel scope narrowing: complement base events are emitted,
	// attributed to the broad policy (its unscoped rule bit survives every filter dimension).
	matchedNone, noneCount := exitPoliciesForComm(buf, commNone)
	require.Positive(t, noneCount,
		"complement base events must be emitted via the broad policy (union defeat)")
...
// commA and the policy emits the commA base event attributed to itself; the complement keeps no rule bit
// and is emitted to no policy.
func Test_PolicyAndDetectorSameScope(t *testing.T) {
	...
	require.Zero(t, none,
		"complement emitted: the kernel failed to clear pol-a's bit (identical scopes collapse to comm=A, "+
			"and policy-level comm has no userland backstop)")
}
...
// complement (neither comm) keeps neither bit, so it is emitted to no policy.
func Test_PolicyAndDetectorDifferentScope(t *testing.T) {
	...
	// The complement matched neither the detector's nor the policy's scope: pol-b's bit must have been
	// cleared in the kernel (policy-level comm has no userland backstop), so it is emitted to no policy.
	_, none := exitPoliciesForComm(buf, commNone)
	require.Zero(t, none, "complement emitted: the union kernel filter {A,B} let a neither-comm exit keep pol-b's bit")
}
