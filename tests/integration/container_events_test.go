...
func Test_ContainerCreateRemove(t *testing.T) {
	...
	// Start Tracee
	traceeInstance, err := startTracee(ctx, t, cfg, nil, nil, false)
	...
	for _, evt := range capturedEvents {
		...
		case int(events.ContainerCreate):
			...
			require.NotEmpty(t, evt.Args, "ContainerCreate should have arguments")
		...
		case int(events.ContainerRemove):
			...
			require.NotEmpty(t, evt.Args, "ContainerRemove should have arguments")
...
func Test_ExistingContainers(t *testing.T) {
	...
	containerName := "tracee-existing-container"
	...
	startCmd := exec.Command("docker", "run", "--name", containerName, "-d", "busybox", "sleep", "60")
	err = startCmd.Run()
	...
	for _, evt := range capturedEvents {
		if evt.EventID == int(events.ExistingContainer) {
			...
			// Verify it has container-related arguments
			require.NotEmpty(t, evt.Args, "ExistingContainer should have arguments")
