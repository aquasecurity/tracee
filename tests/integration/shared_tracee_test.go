...
func withPolicies(t *testing.T, trc *tracee.Tracee, buf *testutils.EventBuffer, pfs []testutils.PolicyFileWithID, fn func(t *testing.T)) {
	...
	defer func() {
		// Best-effort teardown with assert (not require): a failed removal must still be reported,
		// but it must not Goexit out of this loop and leave the REMAINING policies applied on the
		// shared tracee - that would cascade one root-cause failure into every later subtest
		// (leaked names in Policies.Matched, polluted baselines).
		for i := len(applied) - 1; i >= 0; i-- {
			if assert.NoError(t, trc.RemovePolicy(applied[i]), "remove policy %q", applied[i]) {
				assert.NotContains(t, trc.ListPolicies(), applied[i], "removed policy must not be listed")
			}
		}
