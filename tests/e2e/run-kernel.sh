...
# Verify we're in tracee root directory
if [[ ! -d ./pkg ]] || [[ ! -d ./detectors ]]; then
...
for TEST in $TESTS; do
    ...
    *) ;;
    ...
    # run tracee-tester (triggering the detector)
...
# Map detector IDs to their produced event names
# Detectors produce events with a custom "name" field (e.g., "anti_debugging")
# while their detector ID might be different (e.g., "TRC-102")
# This mapping is needed to search for detection events by their event name
...
found=0
    ...
    if [[ -z "$EVENT_NAME" ]]; then
        error_exit "$TEST: detector ID not found in detector_to_event mapping. Please add it."
    fi
    
    # Search for detector event by its produced event name
