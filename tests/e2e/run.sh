...
for test_name in "${!test_pids_map[@]}"; do
    ...
    # Extract PROCTREE_DATA_STORE detector logs from Tracee log file
    if [[ "${test_name}" == "PROCTREE_DATA_STORE" && -f "${logfile}" ]]; then
        detector_logs=$(grep '\[e2eProcessTreeDataStore\]' "${logfile}" 2> /dev/null || true)
        if [[ -n "${detector_logs}" ]]; then
            info "  PROCTREE_DATA_STORE detector logs from Tracee:"
            while IFS= read -r line; do
                info "    ${line}"
            done <<< "${detector_logs}"
        fi
