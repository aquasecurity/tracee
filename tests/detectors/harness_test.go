...
// newTestExecveEvent creates an execve event for testing
func newTestExecveEvent(pathname string) *v1beta1.Event {
...
func TestDispatchEvent(t *testing.T) {
	...
	// Create input event directly
	input := newTestExecveEvent("/bin/test")

	// Dispatch event
	outputs := harness.DispatchEvent(input)

	// Verify output using testify directly
	require.Len(t, outputs, 1)
	assert.Equal(t, "test_dispatch_event", outputs[0].Name)

	// Verify field value
	val, found := v1beta1.GetData[string](outputs[0], "test_field")
