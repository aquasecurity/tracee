...
Threat selection can be combined with regular events and is performed at policy initialization (static, not runtime filtering):

```console
--events threat.severity=critical --events write
--events fs --events 'threat.severity>=high'
```
