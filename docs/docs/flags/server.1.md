...
**pprof security:** The /debug/pprof/ endpoints are served on the same HTTP address as metrics and healthz with no additional access control. pprof heap dumps can expose sensitive process memory of a privileged eBPF daemon. Keep the default loopback bind, use the NetworkPolicy (Kubernetes), or firewall rules to restrict access. For remote debugging, use an SSH tunnel or kubectl port-forward.

**Kubernetes deployments:** The Helm chart sets `0.0.0.0:3366` explicitly. Enable `networkPolicy.enabled=true` to deploy a NetworkPolicy restricting ingress to Prometheus pods (override `networkPolicy.ingressFrom` if your Prometheus uses different labels). If using TCP gRPC in Kubernetes, set an explicit bind address and ensure NetworkPolicy covers the gRPC port as well.
