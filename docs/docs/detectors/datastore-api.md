...
// Use immediately
proc, found := processStore.GetProcess(entityId)
```
...
type ProcessStore interface {
    ...
    GetProcess(entityId uint64) (*ProcessInfo, error)
...
type ProcessInfo struct {
    EntityID  uint64    // Primary key (hash) - matches Event.Process.EntityId
...
proc, found := d.dataStores.Processes().GetProcess(entityId)
...
proc, found := d.dataStores.Processes().GetProcess(uint64(pid))  // Wrong!
```
...
for _, symbol := range symbols {
    ...
    if symbol.Module != "system" {
        ...
        detection.Data = []*v1beta1.EventData{
...
func (d *MyDetector) OnEvent(ctx context.Context, event *v1beta1.Event) ([]*v1beta1.Event, error) {
    ...
    container, found := d.dataStores.Containers().GetContainer(containerID)
    if !found {
...
```go
func (d *ContainerEscapeDetector) analyzeEscape(event *v1beta1.Event) ([]*v1beta1.Event, error) {
...
func (d *MyDetector) Init(params detection.DetectorParams) error {
    ...
    if d.processStore == nil {
        return fmt.Errorf("process store required")
    }

    // Optional stores can be nil
...
func (d *MyDetector) OnEvent(ctx context.Context, event *v1beta1.Event) ([]*v1beta1.Event, error) {
    ...
    proc, found := d.processStore.GetProcess(entityId)
    container, found := d.containerStore.GetContainer(containerID)
...
- Read the [Detector Developer Guide](developer-guide.md) for complete detector development
- Study real detector implementations in `detectors/` directory
- Explore example detectors for usage patterns
