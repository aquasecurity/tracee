...
output:
  extract_fields:
    - name: output_field_name   # Name in output event
      source: data.input_field  # Source path in input event
...
output:
  extract_fields:
    - name: binary_path
      source: data.binary_path  # From base detector
    - name: container_id
      source: workload.container.id
```
...
output:
  extract_fields:
    - name: filepath
      source: data.pathname
    - name: process
      source: workload.process.executable.path
```
...
output:
  extract_fields:
    - name: container_id
      source: workload.container.id
    - name: container_name
      source: workload.container.name
    - name: image
      source: workload.container.image.name
    - name: command
      source: data.pathname
```
...
- **Field extraction**: Verifies source paths are supported
...
- Verify source path exists in input event
