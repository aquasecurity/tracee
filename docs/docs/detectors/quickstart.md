...
func (d *MyDetector) OnEvent(event *v1beta1.Event) {
    ...
    containerName := event.Workload.Container.Name
    containerImage := event.Workload.Container.Image
...
**Pros**: Simple, direct field access
**Cons**: Cannot handle missing/not-found containers gracefully
...
func (d *MyDetector) OnEvent(event *v1beta1.Event) {
    ...
    if strings.Contains(container.Image, "malicious") {
        // detect...
    }
}
```
...
**Pros**: Flexible error handling, access to additional datastore methods
