...
| `threat.mitre.technique` | MITRE technique ID | T1055, T1071 | =, != (supports wildcards) |
| `threat.mitre.tactic` | MITRE tactic name | Defense Evasion, Execution | =, != (supports wildcards) |
| `threat.name` | Threat identifier | process_injection | =, != (supports wildcards) |

**Note:** Threat-based selection is static - detectors are enabled at policy initialization time based on their metadata, not filtered at runtime.
