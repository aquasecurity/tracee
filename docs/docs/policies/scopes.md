...
Network packet and flow events (`net_packet_*`, `net_flow_*`) carry the process context of the
task that owned the underlying socket **when the socket was first tracked** — not of the task
that happens to send or receive each packet. Process-context scopes (`uid`, `pid`, `comm`,
`mntns`, `pidns`, `executable`, etc.) are evaluated per event in user space against that
socket-bound context and the **current** policy set (so runtime policy changes apply to
existing sockets), but the context itself is frozen at tracking time.

Two consequences follow:

- A long-lived socket keeps the scope identity it had when it was first tracked (the original
  owner's comm/uid/...), even if later packets are driven by a different task.
