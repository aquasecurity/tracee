...
type Event struct {
	...
	// BREAKING CHANGE (matched-rules model): these three fields replace the previous
	// PoliciesVersion uint64 / MatchedPoliciesKernel uint64 / MatchedPoliciesUser uint64. The
	// per-event model tracks matched RULES (a bitmap array, so rule IDs may exceed 64), not a
	// single per-policy word. types/trace is a separately-published Go module, so out-of-tree
	// consumers that referenced the old names/types must update. The JSON wire format is
	// UNCHANGED - all three are `json:"-"`, and MatchedPolicies (policy names) still marshals as
	// `matchedPolicies` - so JSON consumers are unaffected. Warrants a major-version bump +
	// release note for the types module.
