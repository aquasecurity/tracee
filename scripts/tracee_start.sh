...
Options:
    ...
    --workdir, -w PATH       Working directory for tracee (default: ${TRACEE_INSTALL_PATH_DEFAULT})
...
Note: All outputs are automatically configured in JSON format.
      ...
      The script manages general workdir, output, and log flags internally before passing them to tracee.
      Do not pass --general, --output or --log flags as additional arguments as they will conflict
...
Examples:
    ...
    $0 --bin ~/bin/tracee --workdir /tmp/tracee_install -e "openat"
...
general_flag="--general workdir=${TRACEE_INSTALL_PATH}"
...
info "To stop Tracee, run: ./scripts/tracee_stop.sh -w ${TRACEE_INSTALL_PATH}"
