...
- **scope filters** - `comm`, `uid`, `pid` (host), `mntns`, `pidns`, `uts`, `container`, `executable` -
  whether written in policy `spec.scope` OR in a rule's `filters:` (`tree` and a specific container id
  are kernel-enforced from `spec.scope` only), and
