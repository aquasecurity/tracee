	...
	for _, testCase := range testCases {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			for i := 0; i < len(testCase.events); i++ {
				evt := testCase.events[i]
				newSorter.addEvent(&evt)
			}
...
func TestEventsChronologicalSorter_Start(t *testing.T) {
	...
	for _, testCase := range testCases {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			outputChan := make(chan *pipeline.Event)
...
func retrieveEventsFromSorter(expectedEventsAmount int, sorterOutputChan <-chan *pipeline.Event, fatalErrorsChan chan error) ([]pipeline.Event, error) {
	ticker := time.NewTicker(2 * time.Second)
	outputList := make([]pipeline.Event, 0)
	eventsReceived := 0
	for {
		select {
...
func sendTestEvents(eventPool []eventsIteration) chan *pipeline.Event {
	inputChan := make(chan *pipeline.Event)
	go func() {
