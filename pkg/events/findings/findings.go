...
func newEvent(id int, f *detect.Finding, e trace.Event) *trace.Event {
	...
	return &trace.Event{
		...
		// MatchedRulesKernel/User are deliberately NOT inherited: rule-bit indexes are only
		// meaningful within one event's rule table, and this event carries the SIGNATURE's
		// EventID while e's bitmap is keyed by the base event's rule IDs. The signature
		// engine stage remaps the base bitmap onto the signature event's own rule IDs
		// (GetDerivedEventMatchedRules) before the event enters the pipeline.
