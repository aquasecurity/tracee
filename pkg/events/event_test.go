...
func TestEventGetIDAndID32Bit_MultipleThreads(t *testing.T) {
	...
	for i := 0; i < amountOfTestThreads; i++ {
		...
		go func() {
			...
			require.Equal(t, ID(12345), id)
			require.Equal(t, ID(12345), id32Bit)
			wg.Done()
		}()
	}
...
func TestEventSetAndGetID32Bit_MultipleThreads(t *testing.T) {
	...
	require.Condition(t, func() bool {
		return event.GetID32Bit() < ID(amountOfTestThreads)
	})
}
...
func TestEventSetAndGetNameAndDocPath_MultipleThreads(t *testing.T) {
	...
	require.Condition(t, func() bool {
		...
		return n < amountOfTestThreads && d < amountOfTestThreads // only testing races here, not values
	})
}
...
func TestEventSetDependencies_MultipleThreads(t *testing.T) {
	...
	require.Equal(t, event.GetDependencies(), dependency)
}
...
func TestEventSetAndGetDependencies_MultipleThreads(t *testing.T) {
	...
	require.True(t, true, true) // only testing races (for new pointers) here, not values
}
...
func TestEventSetAndGetSets_MultipleThreads(t *testing.T) {
	...
	require.Equal(t, event.GetSets(), sets)
}
