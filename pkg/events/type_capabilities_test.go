...
// are in this file (and not the dep_capabilities_test.go file, which only tests atomic setters
// and getters for the Capabilities instance).
//
...
func TestCapabilities_Multiple_Capabilities_And_Rings(t *testing.T) {
	...
	for _, testCase := range testCases {
		...
		for _, ringType := range testCase.RingTypes {
			require.ElementsMatch(t, testCase.Caps, c.GetCaps(ringType))
		}
		for _, ringType := range testCase.RingTypes {
