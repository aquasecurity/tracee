...
func ParseMmapProt(rawValue uint64) MmapProtArgument {
	...
	if rawValue == PROT_NONE.Value() {
		...
		if optionsAreContainedInArgument(rawValue, PROT_READ.Value()) {
			f = append(f, PROT_READ.String())
		}
		if optionsAreContainedInArgument(rawValue, PROT_WRITE.Value()) {
			f = append(f, PROT_WRITE.String())
		}
		if optionsAreContainedInArgument(rawValue, PROT_EXEC.Value()) {
			f = append(f, PROT_EXEC.String())
		}
		if optionsAreContainedInArgument(rawValue, PROT_SEM.Value()) {
			f = append(f, PROT_SEM.String())
		}
		if optionsAreContainedInArgument(rawValue, PROT_GROWSDOWN.Value()) {
			f = append(f, PROT_GROWSDOWN.String())
		}
		if optionsAreContainedInArgument(rawValue, PROT_GROWSUP.Value()) {
