...
type SymbolTable[T Symbol] struct {
	mu sync.RWMutex
...
func NewSymbolTable[T Symbol](lazyNameLookup bool) *SymbolTable[T] {
...
func (st *SymbolTable[T]) ForEachSymbol(callback func(symbol *T)) {
	...
	for i := range len(st.sortedSymbols) {
		callback(st.sortedSymbols[i])
	}
}

func (st *SymbolTable[T]) Clear() {
	st.mu.Lock()
	defer st.mu.Unlock()

	st.sortedSymbols = make([]*T, 0)
	clear(st.symbolsByName)
}
