...
type KernelSymbolTable struct {
	...
	// Used for managing required data symbols
	requiredDataSymbolsOnly  bool
	requiredDataSymbols      map[string]struct{}
	requiredDataSymbolsMutex sync.RWMutex
}

// Creates a new KernelSymbolTable.
...
// If requiredDataSymbolsOnly is true, data symbols will be added only if they
// were explicitly selected using `AddRequiredDataSymbols()`.
func NewKernelSymbolTable(lazyNameLookup bool, requiredDataSymbolsOnly bool) *KernelSymbolTable {
	return &KernelSymbolTable{
		...
		requiredDataSymbolsOnly: requiredDataSymbolsOnly,
		requiredDataSymbols:     make(map[string]struct{}),
	}
}

func (kst *KernelSymbolTable) symbolFromInternal(symbol *kernelSymbolInternal) *KernelSymbol {
	kst.ownersMutex.RLock()
	defer kst.ownersMutex.RUnlock()
...
// Add a list of symbol names to the list of required data symbols.
// The next time `Update()` will be called, they will be added to the symbol
// table (if they exist).
// The symbol names don't have to be of data symbols. If other symbol types
// are present they will have no effect on the update logic.
func (kst *KernelSymbolTable) AddRequiredDataSymbols(symbolNames []string) {
	if !kst.requiredDataSymbolsOnly {
		return
	}

	kst.requiredDataSymbolsMutex.Lock()
	defer kst.requiredDataSymbolsMutex.Unlock()

	for _, name := range symbolNames {
		kst.requiredDataSymbols[name] = struct{}{}
	}
}

// Read the contents of /proc/kallsyms and update the symbol table.
func (kst *KernelSymbolTable) Update() error {
	...
	return kst.UpdateFromReader(file)
}

// Read the contents of the given buffer and update the symbol table
func (kst *KernelSymbolTable) UpdateFromReader(reader io.Reader) error {
	kst.requiredDataSymbolsMutex.RLock()
	defer kst.requiredDataSymbolsMutex.RUnlock()
	...
	for scanner.Scan() {
		...
		if strings.ContainsAny(symbolType, "DdBbRr") && kst.requiredDataSymbolsOnly {
			if _, exists := kst.requiredDataSymbols[symbolName]; !exists {
				continue
			}
		}
