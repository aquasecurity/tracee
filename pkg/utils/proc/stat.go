...
var procStatCommRegex = regexp.MustCompile(`\([^)]+\)`)
...
func newProcStat(filePath string) (*ProcStat, error) {
	...
	// replace spaces in comm with dots (so it can be parsed as a single field)
	statBytes = procStatCommRegex.ReplaceAllFunc(statBytes,
		func(s []byte) []byte {
