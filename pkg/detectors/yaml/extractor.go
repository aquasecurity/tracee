...
func NewFieldExtractor(spec ExtractFieldSpec) (FieldExtractor, error) {
	...
	if spec.Source == "" {
		return nil, errfmt.Errorf("field source cannot be empty")
	}

	// Parse the source path
	path := strings.Split(spec.Source, ".")
	if len(path) == 0 {
		return nil, errfmt.Errorf("invalid source path '%s'", spec.Source)
	}
...
func (e *fieldExtractor) Extract(event *v1beta1.Event) (*v1beta1.EventValue, error) {
	...
	default:
		return nil, errfmt.Errorf("unsupported root path '%s' in source '%s'", root, e.source)
	}
}
...
func (e *fieldExtractor) extractFromData(event *v1beta1.Event) (*v1beta1.EventValue, error) {
	if len(e.path) < 2 {
		return nil, errfmt.Errorf("data path requires at least 2 segments: %s", e.source)
	}
...
func (e *fieldExtractor) extractFromWorkload(event *v1beta1.Event) (*v1beta1.EventValue, error) {
	...
	case "workload.process.pid.value":
		...
		if pid == 0 && !e.optional {
			return nil, errfmt.Errorf("process pid is nil")
		}
	...
	case "workload.process.unique_id.value", "workload.process.entity_id.value":
		...
		if entityID == 0 && !e.optional {
			return nil, errfmt.Errorf("process unique_id is nil")
		}
	...
	case "workload.process.host_pid.value":
		...
		if hostPid == 0 && !e.optional {
			return nil, errfmt.Errorf("process host_pid is nil")
		}
	...
	case "workload.process.executable.path":
		...
		if path == "" && !e.optional {
			return nil, errfmt.Errorf("process executable is nil")
		}
	...
	case "workload.process.real_user.id.value":
		...
		if userId == 0 && !e.optional {
			return nil, errfmt.Errorf("process real_user.id is nil")
		}
	...
	case "workload.process.thread.name":
		...
		if name == "" && !e.optional {
			return nil, errfmt.Errorf("process thread is nil")
		}
	...
	case "workload.process.thread.tid.value":
		...
		if tid == 0 && !e.optional {
			return nil, errfmt.Errorf("thread tid is nil")
		}
	...
	case "workload.process.thread.host_tid.value":
		...
		if hostTid == 0 && !e.optional {
			return nil, errfmt.Errorf("thread host_tid is nil")
		}
	...
	case "workload.process.thread.unique_id.value":
		...
		if entityId == 0 && !e.optional {
			return nil, errfmt.Errorf("thread unique_id is nil")
		}
		return v1beta1.NewUInt32Value(e.name, entityId), nil
	...
	case "workload.container.id":
		...
		if id == "" && !e.optional {
			return nil, errfmt.Errorf("container is nil")
		}
	...
	case "workload.container.image.id":
		...
		if imageId == "" && !e.optional {
			return nil, errfmt.Errorf("container image is nil")
		}
	...
	case "workload.k8s.pod.name":
		...
		if podName == "" && !e.optional {
			return nil, errfmt.Errorf("k8s pod is nil")
		}
	...
	case "workload.k8s.namespace.name":
		...
		if nsName == "" && !e.optional {
			return nil, errfmt.Errorf("k8s namespace is nil")
		}
		return v1beta1.NewStringValue(e.name, nsName), nil

	default:
		return nil, errfmt.Errorf("unsupported workload path '%s'", e.source)
	}
}
...
func BuildExtractors(spec *OutputSpec) ([]FieldExtractor, error) {
	if spec == nil || len(spec.ExtractFields) == 0 {
		return nil, nil
	}

	extractors := make([]FieldExtractor, 0, len(spec.ExtractFields))

	for _, fieldSpec := range spec.ExtractFields {
