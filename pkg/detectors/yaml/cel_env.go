...
func createCELEnvironment(lists map[string][]string, registry datastores.Registry) (*cel.Env, error) {
	envOptions := []cel.EnvOption{
		...
		cel.Macros(
			// Macro 1: getData("field") → getData(event, "field")
			cel.GlobalMacro("getData", 1, getDataMacroExpander),
			// Macro 2: hasData("field") → hasData(event, "field")
			cel.GlobalMacro("hasData", 1, hasDataMacroExpander),
		),

		// Helper function: getData - extract data field from event (returns native type)
		// Usage: getData(event, "pathname") or getData("pathname") via macro
		// Returns: string, int32, uint32, etc. - whatever type the field actually is
		cel.Function("getData",
			cel.Overload("getData_event_string",
				[]*cel.Type{cel.AnyType, cel.StringType},
				cel.DynType, // Returns dynamic type - CEL handles it automatically!
				cel.BinaryBinding(getDataBinding),
			),
		),
...
// getDataBinding extracts a data field from event.Data and returns its native type
func getDataBinding(lhs, rhs ref.Val) ref.Val {
	// lhs is the event (protobuf)
	event, ok := lhs.Value().(*v1beta1.Event)
	if !ok {
		return types.NewErr("getData: first argument must be an Event")
	}

	fieldName, ok := rhs.Value().(string)
	if !ok {
		return types.NewErr("getData: second argument must be a string")
	}
...
// getDataMacroExpander rewrites getData("field") → getData(event, "field")
func getDataMacroExpander(eh parser.ExprHelper, target ast.Expr, args []ast.Expr) (ast.Expr, *common.Error) {
	if len(args) != 1 {
		return nil, eh.NewError(target.ID(), "getData macro requires exactly 1 argument")
	}

	// Create: getData(event, args[0])
	eventIdent := eh.NewIdent("event")
	return eh.NewCall("getData", eventIdent, args[0]), nil
}
