...
func validateOutput(spec *OutputSpec, declaredFields []EventFieldSpec, filePath string) error {
	...
	for _, field := range spec.ExtractFields {
		if field.Name == "" {
			return errfmt.Errorf("%s: output field name is required", filePath)
		}

		if field.Source == "" {
			return errfmt.Errorf("%s: output field '%s' source is required", filePath, field.Name)
		}
		...
		if err := validateExtractionPath(field.Source); err != nil {
	...
	if len(declaredFields) > 0 {
		...
		for _, extracted := range spec.ExtractFields {
			...
			// Type checking would require knowing the source field type from Event protobuf
...
func validateExtractionPath(path string) error {
	...
	root := parts[0]
	validRoots := []string{"data", "workload", "timestamp", "id", "name", "policies"}
	isValid := false
	...
	if !isValid {
		return errfmt.Errorf("invalid extraction path '%s': root must be one of %v", path, validRoots)
	}
