...
type mockProcessStore struct {
	processes map[uint64]*datastores.ProcessInfo
}
...
func (m *mockProcessStore) GetAncestry(entityId uint64, maxDepth int) ([]*datastores.ProcessInfo, error) {
	...
	for i := 0; i < maxDepth; i++ {
		...
		currentEntityId = uint64(proc.PPID)
	}
...
func TestProcessGetAncestryFunction(t *testing.T) {
	registry := &mockRegistry{
		processStore: &mockProcessStore{
			processes: map[uint64]*datastores.ProcessInfo{
				1:   {EntityID: 1, PID: 1, PPID: 0, Name: "init"},
				100: {EntityID: 100, PID: 100, PPID: 1, Name: "parent"},
				200: {EntityID: 200, PID: 200, PPID: 100, Name: "child"},
			},
		},
	}
...
func TestProcessGetChildrenFunction(t *testing.T) {
	registry := &mockRegistry{
		processStore: &mockProcessStore{
			processes: map[uint64]*datastores.ProcessInfo{
				100: {EntityID: 100, PID: 100, PPID: 1, Name: "parent"},
				200: {EntityID: 200, PID: 200, PPID: 100, Name: "child1"},
				300: {EntityID: 300, PID: 300, PPID: 100, Name: "child2"},
			},
		},
	}
...
func TestContainerGetFunction(t *testing.T) {
	...
	result, err = EvaluateExpression(prog, &v1beta1.Event{}, nil, 5)
	require.NoError(t, err)
	assert.Equal(t, true, result)
}
...
func TestDatastoreComplexConditions(t *testing.T) {
	registry := &mockRegistry{
		processStore: &mockProcessStore{
			processes: map[uint64]*datastores.ProcessInfo{
				1:   {EntityID: 1, PID: 1, Name: "init", Exe: "/sbin/init"},
				100: {EntityID: 100, PID: 100, PPID: 1, Name: "bash", Exe: "/bin/bash"},
				200: {EntityID: 200, PID: 200, PPID: 100, Name: "nc", Exe: "/usr/bin/nc"},
			},
		},
		containerStore: &mockContainerStore{
			containers: map[string]*datastores.ContainerInfo{
