...
func (d *YAMLDetector) Init(params detection.DetectorParams) error {
	...
	if d.logger != nil {
		d.logger.Infow("YAML detector initialized",
			"detector_id", d.id,
			"source", d.source,
			"event", d.eventName,
		)
...
// OnEvent processes an event and returns zero or more detection events
func (d *YAMLDetector) OnEvent(ctx context.Context, event *v1beta1.Event) ([]*v1beta1.Event, error) {
	// Create output event based on input event
	output := v1beta1.CreateEventFromBase(event)
	...
	return []*v1beta1.Event{output}, nil
}
