...
func (d *dispatcher) dispatchToDetectors(ctx context.Context, inputEvent *v1beta1.Event, registry *registry) ([]*v1beta1.Event, error) {
	...
	for _, detectorID := range detectorIDs {
		...
		// Post-process detector outputs: assign IDs
		for _, event := range detectorOutputs {
			// Engine assigns event ID and name from the detector's registered event
			event.Id = detector.eventID
			event.Name = detector.eventName

			// TODO: Apply auto-population (Threat, DetectedFrom fields) based on detector definition
