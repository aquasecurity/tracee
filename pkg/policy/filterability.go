...
func classifyRule(rule *EventRule) RuleFilterInfo {
	...
	if ds := rule.DetectorScopeFilter; ds != nil && ds.Enabled() {
		// Only the dimensions the kernel can actually push narrow submission; a detector may
		// declare any ScopeFilter field (podName, cgroupId, tid, ...), and those are evaluated
		// only at dispatch time in user space. Reporting them as "kernel narrows by" would
		// invert the tool's answer, so route the rest to UserScope.
		for _, d := range scopeFilterDims(ds) {
			if perRuleKernelScopeDim[d] {
				info.KernelScope = append(info.KernelScope, d+" (detector)")
			} else {
				info.UserScope = append(info.UserScope, d+" (detector)")
			}
		}
	}
	...
	if perRule := ruleDataScope(rule); perRule != nil && perRule.Enabled() {
		...
		for _, d := range scopeFilterDims(perRule) {
			switch {
			case perRuleKernelScopeDim[d] && !covered[d]:
				info.KernelScope = append(info.KernelScope, d+" (rule)")
			case d == "executable":
				// Binary has no user-space evaluation; when the policy scope also sets it, the
				// two are folded into ONE kernel filter (combinedScopeBinary), so a per-rule
				// executable is kernel-enforced either way - never a user-space filter.
				info.KernelScope = append(info.KernelScope, d+" (rule)")
			default:
				info.UserScope = append(info.UserScope, d)
			}
		}
		// A per-rule containerId= implies the container BOOLEAN (kernel-narrowable, reported
		// above as "container"), but the specific-id equality itself is refined in USER SPACE
		// only - surface it so the report doesn't overstate the kernel's narrowing.
		if perRule.ContainerID().Enabled() {
			info.UserScope = append(info.UserScope, "containerId")
		}
	}
