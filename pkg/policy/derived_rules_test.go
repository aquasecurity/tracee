...
func buildEventRules(rules ...*EventRule) EventRules {
	...
	for _, r := range rules {
		...
		// Mirror assignStableRuleIDs' cached all-rules bitmap so GetAllRulesBitmap works on
		// manually-built EventRules (tests bypass the manager's rebuild path).
		bitwise.SetBitInArray(&er.allRulesBitmap, r.ID)
	}
