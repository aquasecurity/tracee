...
func (ps *policies) computePoliciesConfig() *PoliciesConfig {
	...
	for _, p := range ps.allFromMap() {
		...
		if p.Rules.EnabledDataPrefixMatch() {
			cfg.DataFilterPrefixEnabled |= 1 << offset
		}
		if p.Rules.EnabledDataSuffixMatch() {
			cfg.DataFilterSuffixEnabled |= 1 << offset
		}
		if p.Rules.EnabledDataExactMatch() {
		...
		if p.Rules.MatchIfKeyMissingDataExactMatch() {
			cfg.DataFilterExactMatchIfKeyMissing |= 1 << offset
		}
		if p.Rules.MatchIfKeyMissingDataPrefixMatch() {
			cfg.DataFilterPrefixMatchIfKeyMissing |= 1 << offset
		}
		if p.Rules.MatchIfKeyMissingDataSuffixMatch() {
