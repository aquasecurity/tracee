...
func Test_updateRulesForEvent_KeepsDependencyRuleInUserland(t *testing.T) {
	...
	// Data belongs to some dependent/derived event, and it carries a scope filter. The rebuild
	// keeps only dependency rules whose chain is ALIVE (owning policy still selects the consumer
	// event), so model a realistic chain: an owner policy selecting the consumer.
	depScope := filters.NewScopeFilter()
	require.NoError(t, depScope.Parse("comm", "=bash"))
	consumerID := events.ID(1 << 20)
	owner := NewPolicy()
	owner.Name = "owner"
	owner.Rules[consumerID] = RuleData{EventID: consumerID}
	depRule := &EventRule{
		ID:            0,
		Data:          &RuleData{EventID: consumerID, ScopeFilter: depScope},
		Policy:        owner,
	...
	tempPolicies := map[string]*Policy{pol.Name: pol, owner.Name: owner}
...
func Test_updateRulesForEvent_OverflowBoundary(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// Each dep rule needs a live chain (owning policy selecting a distinct consumer
			// event), or the rebuild would rightly discard it as a dead chain.
			nDeps := tc.total - 1
			owner := NewPolicy()
			owner.Name = "owner"
			...
			for i := 0; i < nDeps; i++ {
				consumerID := events.ID(1<<20 + i)
				owner.Rules[consumerID] = RuleData{EventID: consumerID}
				r := &EventRule{
					ID:            uint(i),
					Data:          &RuleData{EventID: consumerID},
					Policy:        owner,
					SelectionType: SelectedByDependency,
				}
				depRules[i] = r
...
// Test_updateRulesForEvent_PreservesDisabledRules guards the runtime DisableRule state across
// rebuilds: rule IDs are stable, so the disabled bitmap must survive a rebuild triggered by an
// UNRELATED policy change (dropping it would silently re-enable other policies' disabled rules),
// while assignStableRuleIDs prunes bits whose rule retired (a reused ID must start enabled).
func Test_updateRulesForEvent_PreservesDisabledRules(t *testing.T) {
	t.Parallel()
	...
	require.NotEmpty(t, pm.rules[events.Openat].disabledRules, "guard: rule disabled")
	var p1RuleID uint
	for _, r := range pm.rules[events.Openat].Rules {
		if r.Policy != nil && r.Policy.Name == "p1" {
			p1RuleID = r.ID
		}
	}

	// An UNRELATED policy touching the same event rebuilds its rules; p1's disabled bit must survive.
	p2 := NewPolicy()
	p2.Name = "p2"
	p2.Rules = map[events.ID]RuleData{events.Openat: newRuleData(events.Openat)}
	require.NoError(t, pm.AddPolicy(p2))

	disabled := pm.rules[events.Openat].disabledRules
	require.True(t, bitwise.HasBitInArray(disabled, p1RuleID),
		"p1's DisableRule state was reset by an unrelated policy change")

	// Removing p1 retires its rule ID; the stale disabled bit must be pruned so a future
	// occupant of that ID starts enabled.
	require.NoError(t, pm.RemovePolicy("p1"))
	require.False(t, bitwise.HasBitInArray(pm.rules[events.Openat].disabledRules, p1RuleID),
		"retired rule's disabled bit must be pruned (a reused ID would inherit it)")
}
