...
type filterMaps struct {
	...
	extendedScopeFilterConfigs map[filterVersionKey]extendedScopeFiltersConfig
}
...
type FilterMaps struct {
	...
	BinaryFilters map[FilterVersionKey]map[filters.NSBinary][]RuleBitmap
	// ExtendedScopeFilterConfigs is keyed by (version, eventID) like the value maps above:
	// an in-flight event carries the rules version it was generated under, and applying a
	// NEWER config against version-keyed value maps that no longer hold that version would
	// force key-missing verdicts and clear legitimate bits.
	ExtendedScopeFilterConfigs map[FilterVersionKey]ExtendedScopeFiltersConfig
}
...
func (pm *PolicyManager) computeFilterMaps(
	...
	for eventID, eventRules := range pm.rules {
		...
		maps.extendedScopeFilterConfigs[vKey] = pm.computeScopeFiltersConfig(eventID)
	}
	return maps, nil
}
...
func (pm *PolicyManager) processRuleScopeFilters(
	...
	// BinaryFilters: the kernel-effective binary filter is the AND of EVERY enabled source (policy
	// scope, detector-declared scope, per-rule scope). Binary is kernel-enforced only - matchPolicies
	// runs before the binary path is resolvable, so there is no user-space backstop - which means a
	// first-enabled-source-wins choice (used by the other dimensions, where userland re-ANDs the rest)
	// would silently enforce a per-rule/detector executable= NOWHERE whenever the policy scope also
	// sets one. combinedScopeBinary folds the sources instead.
	binNotEqual, binEqual := combinedScopeBinary(rule)
	updateRuleBitmapsForEvent(filterMaps.binaryFilters, vKey, rule.ID, binNotEqual, binEqual)

	return nil
}

// scopeBinarySources returns the ENABLED binary scope filters of a rule in precedence order
// (policy scope, detector-declared scope, per-rule scope). Callers guarantee rule.Policy != nil.
func scopeBinarySources(rule *EventRule) []*filters.BinaryFilter {
	var srcs []*filters.BinaryFilter
	if f := rule.Policy.BinaryFilter; f.Enabled() {
		srcs = append(srcs, f)
	}
	if ds := rule.DetectorScopeFilter; ds != nil && ds.Binary().Enabled() {
		srcs = append(srcs, ds.Binary())
	}
	if pr := ruleDataScope(rule); pr != nil && pr.Binary().Enabled() {
		srcs = append(srcs, pr.Binary())
	}
	return srcs
}

// combinedScopeBinary folds a rule's enabled binary scope sources into ONE kernel filter whose
// verdict for any binary equals the AND of all sources' verdicts. A binary filter deviates from
// its MatchIfKeyMissing default only on finitely many keys, so the AND is computed exactly over
// the union of all sources' keys.
//
// Namespace subtlety: the kernel (and the userland mirror, see binaryBitmaps) tries the
// any-namespace key ({MntNS: 0, Path}) BEFORE the ns-specific one, so when any source pins a
// path to specific namespaces, the combined filter emits only ns-specific entries for that path -
// an any-ns entry would shadow them. Other namespaces then fall through to key-missing, which
// resolves to the combined default (see combinedScopeBinaryMatchIfMissing).
func combinedScopeBinary(rule *EventRule) (notEqual, equal map[filters.NSBinary]struct{}) {
	srcs := scopeBinarySources(rule)

	switch len(srcs) {
	case 0:
		return nil, nil
	case 1: // single source: push its equalities verbatim (the common case, no fold needed)
		eqs := srcs[0].Equalities()
		return eqs.NotEqual, eqs.Equal
	}

	type sourceEqs struct {
		equal, notEqual map[filters.NSBinary]struct{}
		mim             bool
	}
	all := make([]sourceEqs, 0, len(srcs))
	keys := make(map[filters.NSBinary]struct{})
	nsSpecificPaths := make(map[string]bool) // paths some source pins to a specific mnt ns
	for _, f := range srcs {
		eqs := f.Equalities()
		all = append(all, sourceEqs{equal: eqs.Equal, notEqual: eqs.NotEqual, mim: f.MatchIfKeyMissing()})
		for k := range eqs.Equal {
			keys[k] = struct{}{}
			if k.MntNS != 0 {
				nsSpecificPaths[k.Path] = true
			}
		}
		for k := range eqs.NotEqual {
			keys[k] = struct{}{}
			if k.MntNS != 0 {
				nsSpecificPaths[k.Path] = true
			}
	...
	// eval mirrors the kernel's per-filter lookup order: the any-ns entry decides first, then
	// the ns-specific one, else the filter's default.
	eval := func(f sourceEqs, k filters.NSBinary) bool {
		anyNS := filters.NSBinary{Path: k.Path}
		if _, ok := f.equal[anyNS]; ok {
			return true
		}
		if _, ok := f.notEqual[anyNS]; ok {
			return false
		}
		if k.MntNS != 0 {
			if _, ok := f.equal[k]; ok {
				return true
			}
			if _, ok := f.notEqual[k]; ok {
				return false
			}
		}
		return f.mim
	...
	equal = make(map[filters.NSBinary]struct{})
	notEqual = make(map[filters.NSBinary]struct{})
	for k := range keys {
		if k.MntNS == 0 && nsSpecificPaths[k.Path] {
			continue // would shadow this path's ns-specific entries in the kernel double lookup
		}
		verdict := true
		for _, f := range all {
			if !eval(f, k) {
...
// combinedScopeBinaryMatchIfMissing returns the match-if-key-missing flag of the combined binary
// filter: a binary absent from every source's sets matches iff EVERY enabled source defaults to
// match (AND of the sources' flags). With a single enabled source this equals that source's flag,
// preserving the effectiveMatchIfMissing semantics for the common case.
func combinedScopeBinaryMatchIfMissing(rule *EventRule) bool {
	srcs := scopeBinarySources(rule)
	if len(srcs) == 0 {
		return false
	}
	for _, f := range srcs {
		if !f.MatchIfKeyMissing() {
			return false
		}
	}
	return true
