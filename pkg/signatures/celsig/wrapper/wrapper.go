...
func Wrap(envelope protocol.Event) (*Event, error) {
	...
	return &Event{
		...
		ProcessorID:         int64(event.Context.ProcessorID),
		ProcessID:           int64(event.Context.Process.ProcessID),
		CgroupID:            uint64(event.Context.Process.CgroupID),
		ThreadID:            int64(event.Context.Process.ThreadID),
		ParentProcessID:     int64(event.Context.Process.ParentProcessID),
		HostProcessID:       int64(event.Context.Process.HostProcessID),
		HostThreadID:        int64(event.Context.Process.HostThreadID),
		HostParentProcessID: int64(event.Context.Process.HostParentProcessID),
		UserID:              int64(event.Context.Process.UserID),
		MountNS:             int64(event.Context.Process.MountNS),
		PIDNS:               int64(event.Context.Process.PIDNS),
		ProcessName:         event.Context.Process.ProcessName,
		HostName:            event.Context.Process.HostName,
		Container:           wrapContainerData(&event.Context.Container),
		Kubernetes:          wrapKubernetesData(&event.Context.Kubernetes),
		EventID:             int64(event.EventID),
		EventName:           event.EventName,
		ArgsNum:             int64(event.ArgsNum),
		ReturnValue:         int64(event.ReturnValue),
		StackAddresses:      event.Context.StackAddresses,
		Args:                args,
	}, nil
}
