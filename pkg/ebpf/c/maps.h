...
struct data_filter_exact {
    ...
    __type(value, eq_t);
    // Like the other filter prototypes: one instance exists per (version, event) generation,
    // so preallocating 1024 x 260-byte keys per instance (~300KB) would multiply across
...
// max_entries: a BROAD tree root (e.g. tree=1, "everything") needs one entry per live process,
// so this is sized to the same ceiling as proc_info_map (30720). Kept a plain HASH (not LRU):
// LRU eviction would silently drop a live process's tree membership mid-run (an under-match /
// missed detection), whereas a HASH at capacity fails the fork-time bpf_map_update_elem loudly
// enough to debug-log. sched_process_free removes entries, so steady-state occupancy tracks the
// live process count. Exceeding it under an extreme fan-out is a known cap (see the tree docs).
