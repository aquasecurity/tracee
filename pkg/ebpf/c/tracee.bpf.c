...
statfunc int get_mod_tree_iteration_num()
{
    int key = 2;
    int *iteration_num = (int *) bpf_map_lookup_elem(&module_context_map, &key);
...
{
    int key = 1;
    int *idx = (int *) bpf_map_lookup_elem(&module_context_map, &key);
    if (idx == NULL)
        return -1;
    return *idx;
}
...
int lkm_seeker_modtree_loop(struct pt_regs *ctx)
{
    ...
    int iteration_num = get_mod_tree_iteration_num();
    if (iteration_num == -1)
        return -1;
...
#pragma unroll
    ...
    int node_key = 0;
    bpf_map_update_elem(&module_context_map, &node_key, &curr, BPF_ANY);

    int iteration_key = 2;
    bpf_map_update_elem(&module_context_map, &iteration_key, &iteration_num, BPF_ANY);
...
statfunc int find_modules_from_mod_tree(program_data_t *p)
{
    ...
    bpf_map_update_elem(&module_context_map, &node_key, &node, BPF_ANY);

    int idx_key = 1;
    bpf_map_update_elem(&module_context_map, &idx_key, &idx, BPF_ANY);

    int iter_key = 2;
    int iteration_index = 0;
    bpf_map_update_elem(&module_context_map, &iter_key, &iteration_index, BPF_ANY);
