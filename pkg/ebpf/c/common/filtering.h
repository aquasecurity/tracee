...
statfunc u64 match_scope_filters(program_data_t *p)
{
    ...
    // container-started filter (bool; re-added for parity with the per-policy model)
    if (filters_cfg->cont_started_filter_enabled) {
        bool is_started = false;
        if (p->event->context.task.flags & CONTAINER_STARTED_FLAG)
            is_started = true;
        u64 match_bitmap = filters_cfg->cont_started_filter_match_if_key_missing;
        u64 mask = ~filters_cfg->cont_started_filter_enabled;

        res &= bool_filter_matches(match_bitmap, is_started) | mask;
    }

    // process-tree filter (rule model): resolve this process's per-process tree-group
    // membership onto THIS event's rules. Membership is maintained event-agnostically
    // by procfs seeding + fork-time propagation; here we just fold it into the rule
    // bitmap. tree==root rules match when under the root; tree!=root rules match unless
    // under it (the user's `tree=!pid` exclude case). Verifier-validated rule-table lookup.
    // tree + follow rule tables are kept in a separate map (out of the per-CPU
    // event_data_t). Look it up once if either filter is enabled for this event.
    event_tree_config_t *tree_cfg = NULL;
    if (filters_cfg->proc_tree_filter_enabled || filters_cfg->follow_filter_enabled)
        tree_cfg = bpf_map_lookup_elem(&events_tree_config_map, &event_id);

    if (filters_cfg->proc_tree_filter_enabled && tree_cfg != NULL) {
        tree_rule_table_t *tp = &tree_cfg->tree_table;
        eq_t *m = bpf_map_lookup_elem(&proc_tree_membership, &context->host_pid);
        u64 mem_equals = m ? m->equals_in_rules : 0;     // groups whose "=" root covers this pid
        u64 mem_key_used = m ? m->key_used_in_rules : 0; // groups whose any root covers this pid

        // Resolve the per-process, per-group membership onto THIS event's rules, then run it
        // through the same formula as every other equality filter (equality_filter_matches):
        // a rule matches if it is in an "=" subtree, or its key is absent and the filter
        // defaults to match (the tree!= / exclude case). All of a policy's roots fold into one
        // group bit, so the number of roots is unbounded (cap is 64 groups = policies).
        u64 equals_in_rules = 0;
        u64 key_used_in_rules = 0;
        u32 ngroups = tp->num_groups;

#pragma unroll
        for (int i = 0; i < MAX_TREE_GROUPS_PER_EVENT; i++) {
            if ((u32) i >= ngroups)
                break;
            tree_group_rules_t *gp = &tp->groups[i];
            u64 bit = 1ULL << (gp->group & 63); // mask the shift (verifier)
            if (mem_equals & bit)
                equals_in_rules |= gp->rules;
            if (mem_key_used & bit)
                key_used_in_rules |= gp->rules;
        }

        u64 match_if_key_missing = filters_cfg->proc_tree_filter_match_if_key_missing;
        u64 mask = ~filters_cfg->proc_tree_filter_enabled;
        res &= (equals_in_rules | (match_if_key_missing & ~key_used_in_rules)) | mask;
    }

    // follow filter: a process whose ancestry matched a follow-enabled rule keeps being
    // traced by that rule's follow-group even if it no longer matches the scope itself.
    // proc_info->follow_in_scopes is a per-process bitmap of follow-groups, set in
    // evaluate_scope_filters when a follow rule matches and inherited by children via the
    // proc_info copy at fork. Resolve it against this event's follow rules and force-match.
    if (filters_cfg->follow_filter_enabled && tree_cfg != NULL) {
        tree_rule_table_t *fp = &tree_cfg->follow_table;
        u64 membership = p->proc_info->follow_in_scopes;
        u64 followed = 0;
        u32 ngroups = fp->num_groups;
#pragma unroll
        for (int i = 0; i < MAX_TREE_GROUPS_PER_EVENT; i++) {
            if ((u32) i >= ngroups)
                break;
            tree_group_rules_t *gp = &fp->groups[i];
            if (membership & (1ULL << (gp->group & 63)))
                followed |= gp->rules;
        }
        res |= followed & filters_cfg->follow_filter_enabled;
    }
...
statfunc bool evaluate_scope_filters(program_data_t *p)
{
    ...
    p->event->context.matched_rules &= matched_scope_filters;

    // follow: if this event matched any follow-enabled rule, mark the process as followed
    // by those rules' follow-groups so its descendants (which inherit proc_info at fork)
    // keep being traced by them even after they stop matching the scope themselves.
    scope_filters_config_t *fc = &p->event->config.scope_filters;
    u64 matched_follow = p->event->context.matched_rules & fc->follow_filter_enabled;
    if (matched_follow) {
        u32 event_id = p->event->context.eventid;
        event_tree_config_t *tree_cfg = bpf_map_lookup_elem(&events_tree_config_map, &event_id);
        if (tree_cfg != NULL) {
            tree_rule_table_t *fp = &tree_cfg->follow_table;
            u64 add_groups = 0;
            u32 ngroups = fp->num_groups;
#pragma unroll
            for (int i = 0; i < MAX_TREE_GROUPS_PER_EVENT; i++) {
                if ((u32) i >= ngroups)
                    break;
                tree_group_rules_t *gp = &fp->groups[i];
                if (matched_follow & gp->rules)
                    add_groups |= (1ULL << (gp->group & 63));
            }
            if (add_groups)
                p->proc_info->follow_in_scopes |= add_groups;
        }
    }

    return p->event->context.matched_rules != 0 || p->event->config.has_overflow;
}
...
{
    return event->context.matched_rules != 0 || event->config.has_overflow;
}
