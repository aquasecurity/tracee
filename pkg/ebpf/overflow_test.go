...
func TestApplyOverflowScopeFilter(t *testing.T) {
	...
	for _, c := range cases {
		...
		t.Run(c.name, func(t *testing.T) {
			...
			applyOverflowScopeFilter(bitmap, c.bitmaps, c.enabled, c.missing, 1)
...
// TestApplyScopeFilterWordZero covers the fromWord=0 mode used for net base events: their kernel
// bitmap is discarded at decode, so word 0 is narrowed in userland by the same equality formula
// instead of being kernel-owned.
func TestApplyScopeFilterWordZero(t *testing.T) {
	t.Parallel()

	const rule0 = uint64(0b1)
	enabled := []uint64{rule0}
	noMissing := []uint64{0} // equal filter (match only on exact key)

	// key matches -> rule 0 survives in word 0
	bm := []uint64{rule0}
	applyOverflowScopeFilter(bm, []policy.RuleBitmap{{EqualsInRules: rule0, KeyUsedInRules: rule0}}, enabled, noMissing, 0)
	assert.Equal(t, rule0, bm[0], "matching key must keep rule 0 in word 0")

	// key absent -> equal filter fails -> rule 0 cleared from word 0
	bm = []uint64{rule0}
	applyOverflowScopeFilter(bm, nil, enabled, noMissing, 0)
	assert.Equal(t, uint64(0), bm[0], "missing key must clear rule 0 in word 0")

	// same input with fromWord=1 must leave word 0 untouched (overflow mode)
	bm = []uint64{rule0}
	applyOverflowScopeFilter(bm, nil, enabled, noMissing, 1)
	assert.Equal(t, rule0, bm[0], "fromWord=1 must not touch word 0")
}
...
func TestOverflowBinaryNarrowing(t *testing.T) {
	...
	bm := []uint64{0, 0b1}
	applyOverflowScopeFilter(bm, binaryBitmaps(binMap, "/bin/sh", 0), enabled, missing, 1)
	...
	bm = []uint64{0, 0b1}
	applyOverflowScopeFilter(bm, binaryBitmaps(binMap, "/usr/bin/evil", 0), enabled, missing, 1)
