...
//   Handle == cgroupProbe
//
//     Attach(EventHandle, "/sys/fs/cgroup/unified") // to a root cgroup dir
//     Attach(EventHandle, "/sys/fs/cgroup/...")     // specific cgroup dir
//     Detach(EventHandle)
//
...
func Init(module *bpf.Module, netEnabled bool) (Probes, error) {
	...
	allProbes := map[Handle]Probe{
		...
		SockAllocFile:              &traceProbe{eventName: "sock_alloc_file", probeType: kprobe, programName: "sock_alloc_file"},
		SockAllocFileRet:           &traceProbe{eventName: "sock_alloc_file", probeType: kretprobe, programName: "ret_sock_alloc_file"},
