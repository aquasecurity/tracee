...
func (t *Tracee) engineEvents(ctx context.Context, in <-chan *trace.Event) (<-chan *trace.Event, <-chan error) {
	...
	// and return an trace.Event, which should remove the necessity of converting trace.Event to protocol.Event,
	...
	go func() {
		...
		for {
			...
			case event := <-in:
				...
				// if the even is marked as submit is pass it to the engine
