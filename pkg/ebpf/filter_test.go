...
func TestParseProtocolFilters(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			expectedError: errors.New("failed to build args filter: invalid argument filter argument name: bla"),
		},
		{
			testName:      "invalid argfilter 2",
			filters:       []protocol.Filter{protocol.EqualFilter("blabla.bla", "5")},
			expectedError: errors.New("failed to build args filter: invalid argument filter event name: blabla"),
		},
		{
			testName:      "invalid retfilter 1",
			filters:       []protocol.Filter{protocol.EqualFilter("open.retvall", "5")},
			expectedError: errors.New("failed to build args filter: invalid argument filter argument name: retvall"),
		},
		{
			testName:      "adding retval filter then argfilter",
			filters:       []protocol.Filter{protocol.EqualFilter("open.retval", "5"), protocol.EqualFilter("security_file_open.pathname", "/etc/shadow")},
			expectedError: nil,
		},
		{
