...
func (sig *signal) Unmarshal(buffer []byte) error {
	...
	evtDef := events.Definitions.GetEventByID(sig.eventID)
	if evtDef == nil {
		return errfmt.Errorf("failed to get configuration of event %d", sig.eventID)
	}

	sig.args = make([]trace.Argument, len(evtDef.GetParams()))
	err = ebpfDecoder.DecodeArguments(sig.args, int(argnum), evtDef, sig.eventID)
	if err != nil {
