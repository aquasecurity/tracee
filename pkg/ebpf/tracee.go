...
// Default is nil for non-e2e builds.
var populateE2eRegistrations func(*Tracee)
...
func (t *Tracee) ApplyPolicy(p *policy.Policy) (string, error) {
	if p == nil {
		return "", policy.PolicyNilError()
	}
	// Upsert without a lookup-then-act race: try the update first; if the policy does not
	// exist, add it; if a concurrent add won that race, retry the update once. Each manager
	// operation is atomic under its own lock, so the worst case is a spurious retry - never
	// corruption.
	err := t.policyManager.UpdatePolicy(p)
	if errors.Is(err, policy.PolicyNotFoundByNameError(p.Name)) {
		err = t.policyManager.AddPolicy(p)
		if errors.Is(err, policy.PolicyAlreadyExistsError(p.Name)) {
			err = t.policyManager.UpdatePolicy(p)
		}
	}
	if err != nil {
...
// EnableEvent / DisableEvent are LEGACY, predating the runtime policy-change model
// (ApplyPolicy/RemovePolicy). Unlike those, they only flip the userspace snapshot's per-event
// `enabled` gate and start/stop the signature goroutine - they do NOT re-push the kernel filter
// maps (populateFilterMaps), so EnableEvent on an event the kernel is not already submitting will
// not make the kernel start submitting it, and DisableEvent does not stop kernel submission.
// They remain functional and are still exposed over gRPC, but should be REMOVED once the runtime
// policy path (which handles kernel re-push correctly) is proven safe in production; callers
// wanting a runtime change to take effect in the kernel should use ApplyPolicy/RemovePolicy.
