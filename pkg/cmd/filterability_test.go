...
func TestPolicyFilterability_Scenarios(t *testing.T) {
	...
	// Per-rule scope on a kernel-pushable dim (comm) is pushed to the kernel too (not just
	// spec.scope). The rollup shows the BASE dimension (origin qualifiers are collapsed for
	// user-written sources - "comm, comm (rule)" would read as a typo).
...
// TestVerdictTip covers the actionable-tip selection and its cause priority: an always-collected
// (bootstrap) event can never be kernel-gated so it wins, then overflow, then the named broad
// co-selector, then user-space-only filters; kernel verdicts carry no tip.
func TestVerdictTip(t *testing.T) {
	require.Empty(t, verdictTip("kernel", false, nil))

	tip := verdictTip("user-space", true, map[string]bool{"broad": true})
	require.Contains(t, tip, "note:")
	require.Contains(t, tip, "always collects", "bootstrap outranks every other cause (gating is impossible)")

	tip = verdictTip("overflow", false, map[string]bool{"broad": true})
	require.Contains(t, tip, "more than 64 rules", "overflow outranks the broad co-selector")

	tip = verdictTip("user-space", false, map[string]bool{"zzz": true, "aaa": true})
	require.Contains(t, tip, "aaa, zzz", "broad co-selectors are named, sorted")
	require.Contains(t, tip, "restores kernel gating")

	tip = verdictTip("user-space", false, nil)
	require.Contains(t, tip, "only user-space filters")
}
