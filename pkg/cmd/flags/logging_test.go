...
func TestPrepareLogger(t *testing.T) {
	...
	testCases := []struct {
		...
		// valid log aggregate
		{
			...
			logOptions:    []string{"aggregate.enabled"},
			expectedLevel: logger.DefaultLevel,
			expectedAgg:   true,
			expectedFlush: logger.DefaultFlushInterval,
			expectedError: nil,
		...
		// valid log level + aggregate
		{
			...
			logOptions:    []string{"level=debug", "aggregate.enabled"},
			expectedLevel: logger.DebugLevel,
			expectedAgg:   true,
			expectedFlush: logger.DefaultFlushInterval,
			expectedError: nil,
		},
		{
			...
			logOptions:    []string{"level=debug", "aggregate.enabled", "aggregate.flush-interval=10s"},
			expectedLevel: logger.DebugLevel,
			expectedAgg:   true,
			expectedFlush: 10 * time.Second,
			expectedError: nil,
