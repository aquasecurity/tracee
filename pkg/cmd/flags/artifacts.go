...
const (
	...
	artifactFileWrite     = "file-write"
	artifactFileRead      = "file-read"
	artifactExecutable    = "executable"
	artifactKernelModules = "kernel-modules"
	artifactBpfPrograms   = "bpf-programs"
	artifactMemoryRegions = "memory-regions"
	...
	artifactPcap        = "pcap"
	artifactPcapSplit   = "split"
	artifactPcapOptions = "options"
	artifactPcapSnaplen = "snaplen"
...
func (a *ArtifactsConfig) flags() []string {
	...
	// file-write
	if a.FileWrite.Enabled {
		flags = append(flags, fmt.Sprintf("%s.%s", artifactFileWrite, artifactEnabled))
	}
	...
	if len(a.FileWrite.Filters) > 0 {
		for _, filter := range a.FileWrite.Filters {
			flags = append(flags, fmt.Sprintf("%s.%s=%s", artifactFileWrite, artifactFilters, filter))
		}
	} else {
		// Reconstruct from parsed data (for CLI flags that were parsed)
		for _, pathFilter := range a.FileWrite.PathFilter {
			flags = append(flags, fmt.Sprintf("%s.%s=path=%s*", artifactFileWrite, artifactFilters, pathFilter))
		}
	...
	// file-read
	if a.FileRead.Enabled {
		flags = append(flags, fmt.Sprintf("%s.%s", artifactFileRead, artifactEnabled))
	}
	// Output filters from Filters field (for structured configs) or reconstruct from PathFilter/TypeFilter
	if len(a.FileRead.Filters) > 0 {
		for _, filter := range a.FileRead.Filters {
			flags = append(flags, fmt.Sprintf("%s.%s=%s", artifactFileRead, artifactFilters, filter))
		}
	} else {
		// Reconstruct from parsed data (for CLI flags that were parsed)
		for _, pathFilter := range a.FileRead.PathFilter {
			flags = append(flags, fmt.Sprintf("%s.%s=path=%s*", artifactFileRead, artifactFilters, pathFilter))
		}
		// Note: TypeFilter is a bitmask, so we can't easily reconstruct the original filter strings
	}
	...
	// executable
	if a.Executable {
		flags = append(flags, fmt.Sprintf("%s.%s", artifactExecutable, artifactEnabled))
	}

	// kernel-modules
	if a.KernelModules {
		flags = append(flags, fmt.Sprintf("%s.%s", artifactKernelModules, artifactEnabled))
	}

	// bpf-programs
	if a.BpfPrograms {
		flags = append(flags, fmt.Sprintf("%s.%s", artifactBpfPrograms, artifactEnabled))
	}

	// memory-regions
	if a.MemoryRegions {
		flags = append(flags, fmt.Sprintf("%s.%s", artifactMemoryRegions, artifactEnabled))
	}

	// network
	if a.Network.Enabled {
		flags = append(flags, fmt.Sprintf("%s.%s", artifactNetwork, artifactEnabled))
		// Output from Pcap field (for structured configs) or reconstruct from parsed fields
		if a.Network.Pcap.Split != "" {
			flags = append(flags, fmt.Sprintf("%s.%s.%s=%s", artifactNetwork, artifactPcap, artifactPcapSplit, a.Network.Pcap.Split))
		} else {
			...
			if !a.Network.CaptureSingle || hasOtherFlags {
				...
				if len(splitParts) > 0 {
					flags = append(flags, fmt.Sprintf("%s.%s.%s=%s", artifactNetwork, artifactPcap, artifactPcapSplit, strings.Join(splitParts, ",")))
				}
			}
		}
		// Output options
		if a.Network.Pcap.Options != "" {
			flags = append(flags, fmt.Sprintf("%s.%s.%s=%s", artifactNetwork, artifactPcap, artifactPcapOptions, a.Network.Pcap.Options))
		} else if a.Network.CaptureFiltered {
			flags = append(flags, fmt.Sprintf("%s.%s.%s=filtered", artifactNetwork, artifactPcap, artifactPcapOptions))
		}
		// Output snaplen
		if a.Network.Pcap.Snaplen != "" {
			flags = append(flags, fmt.Sprintf("%s.%s.%s=%s", artifactNetwork, artifactPcap, artifactPcapSnaplen, a.Network.Pcap.Snaplen))
			...
			if snaplenStr != "" {
				flags = append(flags, fmt.Sprintf("%s.%s.%s=%s", artifactNetwork, artifactPcap, artifactPcapSnaplen, snaplenStr))
			}
		}
	}
	...
	// dir
	if a.Dir.Path != "" {
		flags = append(flags, fmt.Sprintf("%s.%s=%s", artifactDir, artifactPath, a.Dir.Path))
	}
	if a.Dir.Clear {
		flags = append(flags, fmt.Sprintf("%s.%s", artifactDir, artifactClear))
	}
...
func PrepareArtifacts(artifactsSlice []string) (ArtifactsConfig, error) {
	...
	for _, opt := range artifactsSlice {
		...
		if len(parts) < 2 {
			return ArtifactsConfig{}, errfmt.Errorf("invalid artifacts option: %s, run 'tracee man artifacts' for more info", opt)
		}
...
func parseFileArtifactOptionWrite(fileConfig *FileWriteConfig, subOpt string) error {
	...
	if strings.HasPrefix(subOpt, artifactFilters+"=") {
		filterStr := strings.TrimPrefix(subOpt, artifactFilters+"=")
		if filterStr == "" {
...
func parseNetworkArtifactOption(netConfig *NetworkConfig, subOpt string) error {
	...
	if strings.HasPrefix(subOpt, artifactPcap+".") {
		pcapOpt := strings.TrimPrefix(subOpt, artifactPcap+".")
...
func parseDirArtifactOption(dirConfig *DirConfig, subOpt string) error {
	if subOpt == artifactClear {
		dirConfig.Clear = true
		return nil
	}

	if strings.HasPrefix(subOpt, artifactPath+"=") {
		path := strings.TrimPrefix(subOpt, artifactPath+"=")
		if len(path) == 0 {
