...
func (p tableEventPrinter) Print(event trace.Event) {
	...
	containerImage := event.Context.Container.ImageName
	...
	if p.verbose {
		...
		case ContainerModeDisabled:
			fmt.Fprintf(p.out,
				...
				timestamp,
				event.Context.Process.HostName,
				containerId,
				event.Context.Process.MountNS,
				event.Context.Process.PIDNS,
				event.Context.Process.UserID,
				event.Context.Process.ProcessName,
				event.Context.Process.ProcessID,
				event.Context.Process.ThreadID,
				event.Context.Process.ParentProcessID,
				event.ReturnValue,
				event.EventName,
			)
		case ContainerModeEnabled:
			fmt.Fprintf(p.out,
				...
				event.Context.Process.ProcessName,
				event.Context.Process.ProcessID,
				event.Context.Process.HostProcessID,
				event.Context.Process.ThreadID,
				event.Context.Process.HostThreadID,
				event.Context.Process.ParentProcessID,
				event.Context.Process.HostParentProcessID,
				event.ReturnValue,
				event.EventName,
			)
		case ContainerModeEnriched:
			fmt.Fprintf(p.out,
				...
				event.Context.Process.HostName,
				containerId,
				event.Context.Container.ImageName,
				event.Context.Process.MountNS,
				event.Context.Process.PIDNS,
				event.Context.Process.UserID,
				event.Context.Process.ProcessName,
