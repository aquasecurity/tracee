...
// ContainerID exposes the specific-container-id equality filter (containerId=...). Note it is
// evaluated in USER SPACE only; the container boolean it implies (any container) is what the
// kernel can narrow by.
func (f *ScopeFilter) ContainerID() *StringFilter { return f.containerIDFilter }
