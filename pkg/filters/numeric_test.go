...
func TestNumericFilterNewMethods(t *testing.T) {
	...
	// Test generic filter with uint32
	t.Run("NewNumericFilter uint32", func(t *testing.T) {
		filter := NewNumericFilter[uint32]()
		assert.NotNil(t, filter)
		assert.True(t, filter.is32Bit)
	})
...
func TestNumericFilterTypeSafety(t *testing.T) {
	...
	t.Run("unsigned parsing edge cases", func(t *testing.T) {
		...
		// <=0 should be invalid for unsigned types (extension of <0 check)
		err := filter.Parse("<=0")
		assert.Error(t, err, "<=0 should be invalid for unsigned types")

		// But =0 should be valid
		err = filter.Parse("=0")
