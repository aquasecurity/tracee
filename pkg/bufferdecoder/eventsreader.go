...
// readSunPathFromBuff reads a sun_path string from the `buffer`.
func readSunPathFromBuff(decoder *EbpfDecoder, size int) (string, error) {
	if size <= 0 {
		return "", errfmt.Errorf("size to decode sun_path must be greater than 0")
	}
	...
	decoder.cursor += size - count // move cursor to the end of the sun_path
	return string(res), nil
}
