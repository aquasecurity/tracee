...
func TestHookedSeqOps_GetDefinition(t *testing.T) {
	...
	assert.Equal(t, "hooked_seq_ops", def.ProducedEvent.Fields[0].Name)
}
...
func TestHookedSeqOps_Init(t *testing.T) {
	...
	registry := &mockDataStoreRegistry{symbolStore: symbolStore}
...
func TestHookedSeqOps_OnEvent_SingleHook(t *testing.T) {
	...
	assert.Equal(t, "hooked_seq_ops", output.Data[0].Name)
