			...
			Events: []detection.EventRequirement{
				{
					...
					// Use suffix match to handle paths like /host/proc/kcore
					DataFilters: []string{
						"pathname=*/proc/kcore",
					},
...
func (d *ProcKcoreRead) OnEvent(ctx context.Context, event *v1beta1.Event) ([]detection.DetectorOutput, error) {
	// DataFilter ensures pathname suffix matches (*/proc/kcore), ScopeFilter ensures container origin
	// Only need to check if this is a read operation
