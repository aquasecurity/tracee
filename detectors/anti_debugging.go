			...
			Events: []detection.EventRequirement{
				{
					...
					DataFilters: []string{fmt.Sprintf("request=%d", parsers.PTRACE_TRACEME.Value())},
				},
			},
		},
...
func (d *AntiDebugging) OnEvent(ctx context.Context, event *v1beta1.Event) ([]*v1beta1.Event, error) {
	// If we received this event, it means the data filter matched (request=0/PTRACE_TRACEME)
	// Create detection event directly
	detectionEvent := v1beta1.CreateEventFromBase(event)
