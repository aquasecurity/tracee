...
func (d *HookedSeqOps) Init(params detection.DetectorParams) error {
	...
	// Validate and cache required datastore at initialization
	d.symbolStore = params.DataStores.KernelSymbols()
	if d.symbolStore == nil {
		return errors.New("symbol datastore not available")
	}
...
func (d *HookedSeqOps) OnEvent(ctx context.Context, event *v1beta1.Event) ([]detection.DetectorOutput, error) {
	...
	for i, addr := range seqOpsArr {
		...
		symbols, err := d.symbolStore.ResolveSymbolByAddress(addr)
