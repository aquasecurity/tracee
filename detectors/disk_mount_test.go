...
func TestDiskMount_OnEvent(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:          "mount tmpfs - should not detect",
			devName:       "tmpfs",
			expectedMatch: false,
		},
		{
			name:          "mount proc - should not detect",
			devName:       "proc",
			expectedMatch: false,
		},
		{
			name:          "mount /var/lib - should not detect",
			devName:       "/var/lib/docker/volumes",
			expectedMatch: false,
		},
	}
...
func TestDiskMount_Definition(t *testing.T) {
	...
	assert.Equal(t, "disk_mount_detector", def.ProducedEvent.Name)
