...
func TestKubernetesCertificateTheft(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:           "malicious process reading non-k8s file - should not trigger",
			execPath:       "/usr/bin/malicious",
			pathname:       "/etc/ssl/certs/ca-certificates.crt",
			flags:          0, // O_RDONLY
			expectedOutput: false,
		},
		{
...
func TestKubernetesCertificateTheft_Rename(t *testing.T) {
	...
	event := &v1beta1.Event{
		...
		Data: []*v1beta1.EventValue{
			v1beta1.NewStringValue("old_path", "/etc/kubernetes/pki/apiserver.key"),
		},
	}
