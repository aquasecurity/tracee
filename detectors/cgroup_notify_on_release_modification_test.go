...
func TestCgroupNotifyOnReleaseModification_OnEvent(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:          "different file write - should not detect",
			pathname:      "/sys/fs/cgroup/memory/release_agent",
			flags:         writeFlags,
			expectedMatch: false,
		},
	}
...
func TestCgroupNotifyOnReleaseModification_Definition(t *testing.T) {
	...
	assert.Equal(t, "cgroup_notify_on_release_detector", def.ProducedEvent.Name)
