...
func (d *HookedSyscall) OnEvent(ctx context.Context, event *v1beta1.Event) ([]*v1beta1.Event, error) {
	...
	if err != nil || len(hookedSymbols) == 0 {
		...
		// This matches the original behavior: return [][]interface{}{{syscallName, hexAddress, "", ""}}
	...
	outputEvents := make([]*v1beta1.Event, 0, len(hookedSymbols))
	for _, symbol := range hookedSymbols {
		...
		d.logger.Warnw("Syscall hook detected",
			"syscall", syscallName,
