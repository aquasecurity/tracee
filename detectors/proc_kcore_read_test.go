...
func TestProcKcoreRead_GetDefinition(t *testing.T) {
	...
	assert.Equal(t, "container", def.Requirements.Events[0].ScopeFilters[0])

	// Check produced event
	assert.Equal(t, "proc_kcore_read_detector", def.ProducedEvent.Name)
...
func TestProcKcoreRead_OnEvent_ReadOperation(t *testing.T) {
	...
	testCases := []struct {
		...
		{"wrong_path", "/proc/something", int(parsers.O_RDONLY.Value()), false},
	}
