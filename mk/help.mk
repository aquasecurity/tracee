...
help::
	...
	$(H) '$$ make test-e2e-vm' '# run kernel-tampering E2E tests in a VM (x86_64; Firecracker if /dev/kvm, else QEMU/TCG)'
	...
	$(H) '$$ make tests-e2e' '# run the whole E2E family (core + net + kernel + VM) in one privileged run - ONE sudo prompt'
	$(H) '$$ make tests' '# run EVERY suite (unit + integration + e2e + VM) in one privileged run - ONE sudo prompt'
