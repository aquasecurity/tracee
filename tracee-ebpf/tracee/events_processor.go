...
func (t *Tracee) processEvent(event *trace.TraceeEvent) error {
...
func getEventArgStringVal(event *trace.TraceeEvent, argName string) (string, error) {
...
func getEventArgUint64Val(event *trace.TraceeEvent, argName string) (uint64, error) {
	for _, arg := range event.Args {
		if arg.Name == argName {
...
func getEventArgUint32Val(event *trace.TraceeEvent, argName string) (uint32, error) {
	for _, arg := range event.Args {
		if arg.Name == argName {
...
func getEventArgInt32Val(event *trace.TraceeEvent, argName string) (int32, error) {
	for _, arg := range event.Args {
		if arg.Name == argName {
