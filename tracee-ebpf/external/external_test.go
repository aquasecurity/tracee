	...
	testCases := []testCase{
		{
			...
			"argsnum":4,"returnvalue":0,"args":[{"name":"request","type":"long","value":12345},
			{"name":"pid","type":"pid_t","value":12435},{"name":"data","type":"void*","value":"0xA1"}]}`,
			expect: Event{Timestamp: 26018249532, ProcessID: 12434, ThreadID: 12434, ParentProcessID: 23921,
				HostProcessID: 12434, HostThreadID: 12434, HostParentProcessID: 23921, UserID: 1000, MountNS: 4026531840,
				PIDNS: 4026531836, ProcessName: "strace", HostName: "ubuntu", EventID: 101, EventName: "ptrace", ArgsNum: 4,
				ReturnValue: 0, Args: []Argument{{ArgMeta: ArgMeta{Name: "request", Type: "long"}, Value: int64(12345)},
					{ArgMeta: ArgMeta{Name: "pid", Type: "pid_t"}, Value: int32(12435)}, {ArgMeta: ArgMeta{Name: "data", Type: "void*"}, Value: uintptr(161)}}},
		},
	}
...
func TestArgumentUnmarshalJSON(t *testing.T) {
	...
	testCases := []testCase{
		...
		{
			json:   `{ "name":"test", "type":"const char*", "value": "foo"}`,
			expect: Argument{ArgMeta: ArgMeta{Name: "test", Type: "const char*"}, Value: "foo"},
		},
		{
			json:   `{ "name":"test", "type":"char*", "value": "foo"}`,
			expect: Argument{ArgMeta: ArgMeta{Name: "test", Type: "char*"}, Value: "foo"},
		},
		{
		...
		{
			json:   `{ "name":"test", "type":"err", "value": "err"}`,
			expect: Argument{ArgMeta: ArgMeta{Name: "test", Type: "err"}, Value: "err"},
		},
		{
			json:   `{ "name":"test", "type":"err", "value": 0}`,
			expect: Argument{ArgMeta: ArgMeta{Name: "test", Type: "err"}, Value: "0"},
		},
	}
	for _, tc := range testCases {
		var res Argument
