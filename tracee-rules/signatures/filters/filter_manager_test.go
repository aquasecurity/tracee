...
func TestFilterManager_AddSignature(t *testing.T) {
	...
	require.NoError(t, err, "Adding new signature to the FilterManager")
	filteredSigs, err = filterManager.GetFilteredSignatures(event)
...
func TestFilterManager_RemoveSignature(t *testing.T) {
	...
	require.NoError(t, err, "creating FilterManager with a single signature")
	filteredSigs, err := filterManager.GetFilteredSignatures(event)
	...
	require.NoError(t, err, "Removing the matching signature from the FilterManager")
	filteredSigs, err = filterManager.GetFilteredSignatures(event)
	...
	assert.Error(t, err, "Removing non existing signature from the FilterManager")
}
