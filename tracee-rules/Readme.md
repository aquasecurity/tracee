...
# Rules
Rules are discovered from the local `rules` directory and all it's sub-directories (unless changed by the `--rules-dir` flag). By default, all discovered rules will be loaded unless specific rules are selected using the `--rules` flag.
