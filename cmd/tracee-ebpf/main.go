...
func main() {
	app := &cli.App{
		...
		Action: func(c *cli.Context) error {
			...
			printer, err := tracee.NewEventPrinter(printerConfig.kind, containerMode, cfg.Output.RelativeTime, printerConfig.outFile, printerConfig.errFile)
			if err != nil {
				return err
			}
...
func unpackCOREBinary() ([]byte, error) {
	b, err := tracee_ebpf.BPFBundleInjected.ReadFile("dist/tracee.bpf.core.o")
	if err != nil {
		return nil, err
	}
...
func unpackBPFBundle(dir string) error {
	...
	files, err := tracee_ebpf.BPFBundleInjected.ReadDir(basePath)
	if err != nil {
	...
	for _, f := range files {
		...
		f, err := tracee_ebpf.BPFBundleInjected.Open(filepath.Join(basePath, f.Name()))
		if err != nil {
