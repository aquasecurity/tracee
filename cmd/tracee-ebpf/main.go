	...
	app := &cli.App{
		...
		Flags: []cli.Flag{
			...
			&cli.StringSliceFlag{
				...
				Aliases: []string{"s"},
				Value:   nil,
				Usage:   "select workloads to trace by defining filter expressions. run '--filter help' for more info.",
			},
			&cli.StringSliceFlag{
				Name:    "event",
				Aliases: []string{"e"},
				Usage:   "select events to trace and filter events. run '--event help' for more info.",
			},
			&cli.StringSliceFlag{
				Name:    "capture",
				Aliases: []string{"c"},
