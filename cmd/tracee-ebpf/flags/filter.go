...
// expandFilterField allows filter field aliases
func expandFilterField(field string) string {
	switch field {
	case "e":
		return "event"
	case "c":
...
func parseFilterString(filterStr string) (protocol.Filter, error) {
	...
	// no operator found means a bool filter which is true
	if operatorIndex < 0 {
		filterName = filterStr
		filterName = expandFilterField(filterName)
	...
	// if ! at the start, bool filter which is false
	...
	// if operator is last char then the value is missing
	...
	// new valued filters usually conflict with the expected inputs (currently pids, containerids, etc.)
	// this is why we define that they are a separate field
	if valuesStr == "new" {
		switch operator {
		case protocol.Equal:
	...
	// move string vals into the Values array ([]interface{})
...
func operatorFromString(opStr string) (protocol.FilterOperator, error) {
	...
	case "<":
		return protocol.Lower, nil
	case ">=":
		return protocol.GreaterEqual, nil
	case "<=":
		return protocol.LowerEqual, nil
	}
	//sanity
