...
func init() {
	...
	if stressMode == "" {
		stressMode = fmt.Sprintf("weight=%d", defaultStressWeight)
	}
	stressCmd.Flags().StringVarP(
		...
		"m",
		stressMode,
		"<time=30m|weight=2000000>\tStress mode",
	)
}
...
const (
	...
	defaultStressWeight = uint64(10_000_000)
)
...
func parseStressModeFlag(modeFlag string) (StressModeConfig, error) {
	...
	if len(parts) == 1 {
		switch mode {
		case "time":
			value = fmt.Sprint(defaultStressTime)
		case "weight":
			value = fmt.Sprint(defaultStressWeight)
		default:
			return StressModeConfig{}, fmt.Errorf("invalid stress mode: %s", modeFlag)
		}
	} else {
	...
	case "weight":
		...
		if err != nil {
			return StressModeConfig{}, fmt.Errorf("invalid stress weight: %s", value)
		}
		return StressModeConfig{
			Selected: StressModeWeight,
			Weight:   w,
		}, nil
...
func stressRun(cmd *cobra.Command, args []string) error {
	...
	if cfg.Mode.Selected == StressModeWeight {
		timeLimit = maxStressTime
		fmt.Printf("Stressing the system with weight of %v and time limit of %v\n", humanize.Comma(int64(cfg.Mode.Weight)), timeLimit)
		fmt.Println("Weight mode triggers events based on the weight of the event, until it reaches the event amount or the time limit")
	} else {
		timeLimit = cfg.Mode.Time
		fmt.Printf("Stressing the system without weight for %v\n", timeLimit)
		fmt.Println("Time mode triggers events disregarding the weight, until it reaches the time limit")
	}

	ctx, cancelTimeout := context.WithTimeoutCause(ctx, timeLimit, fmt.Errorf("stress time is up after %v", timeLimit))
	defer cancelTimeout()
	...
	for _, evt := range cfg.Selected.EventsNames {
		_ = evt
		// wg.Add(1)
		// go stressEvent(ctx, evt)
	}

	traceeStatus := RunTracee(ctx, wg, cfg.Selected, []string{}, []string{})
	...
	// unblock all stressEvent goroutines
...
func RunTracee(
	...
	go func() {
		...
		if selected.Origin == "policy" {
			...
			// updatePolicyFiles(selected.PolicyFiles, filterOutCommScope, filterOutPidScope)
			...
			args = append(args, "-s", "comm!=evt")
			for _, outComm := range filterOutCommScope {
				args = append(args, "-s", fmt.Sprintf("comm!=%s", outComm))
			}

			args = append(args, "-s", fmt.Sprintf("pid!=%d", os.Getpid()))
			for _, outPid := range filterOutPidScope {
				args = append(args, "-s", fmt.Sprintf("pid!=%s", outPid))
			}
...
func getFilterOutCommScope() string {
	return fmt.Sprintf("comm!=%s", filepath.Base(os.Args[0]))
}

func getFilterOutPidScope() string {
...
func getEventsAndPoliciesFromPolicyFiles(policyFlags []string) ([]string, []v1beta1.PolicyFile, error) {
	...
	for i := range policyInterfaceSlice {
		...
		if idx := slices.Index(p.Spec.Scope, "global"); idx != -1 {
			p.Spec.Scope[idx] = getFilterOutCommScope()
		} else {
			p.Spec.Scope = append(p.Spec.Scope, getFilterOutCommScope())
		}
		p.Spec.Scope = append(p.Spec.Scope, getFilterOutPidScope())

		// store the modified policy file
