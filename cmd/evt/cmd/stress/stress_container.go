...
func (s *stress) startContainers() error {
	for _, tc := range s.triggers {
		...
		// docker run with full privileges and resources
		...
		checkCmd := exec.CommandContext(s.ctx, "docker", "ps", "-q", "-f", fmt.Sprintf("id=%s", containerID))
		checkOut, _ := checkCmd.Output()
		if len(strings.TrimSpace(string(checkOut))) == 0 {
			// Container already exited - get logs
			logsCmd := exec.CommandContext(s.ctx, "docker", "logs", containerID)
...
func (s *stress) signalContainers() error {
	...
	for _, id := range containerIDs {
		...
		checkCmd := exec.CommandContext(s.ctx, "docker", "ps", "-q", "-f", fmt.Sprintf("id=%s", id))
		output, err := checkCmd.Output()
		if err != nil || len(strings.TrimSpace(string(output))) == 0 {
			// Container not running - get logs to see what happened
			logsCmd := exec.CommandContext(s.ctx, "docker", "logs", id)
			logs, _ := logsCmd.Output()
			return fmt.Errorf(
