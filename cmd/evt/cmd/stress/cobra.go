...
Examples:
  ...
  # Manual Tracee control (don't auto-start)
  evt stress --events ptrace --auto-tracee=false

  # Disable Tracee output (no event file generated)
  evt stress --events security_file_open --tracee-output none

  # Custom output file
  evt stress --events ptrace --tracee-output json:/tmp/my-events.json

  # Disable metrics and pprof for minimal overhead
  evt stress --events security_file_open --metrics=false --pprof=false

  # Enable pyroscope for continuous profiling
  evt stress --events ptrace --pyroscope
...
const (
	...
	defaultStressContainerImage = "evt:latest"
	defaultTraceeBinary         = "./dist/tracee"
	defaultTraceeOutput         = "json:/tmp/tracee-stress.json"
...
func init() {
	...
	stressCmd.Flags().String(
		...
		"<name:tag>\t\tContainer image with evt binary and triggers",
	)

	stressCmd.Flags().Bool(
		"auto-tracee",
		true,
		"\t\t\tAutomatically start Tracee (false = manual, waits for user)",
	)

	stressCmd.Flags().String(
		"tracee-binary",
		defaultTraceeBinary,
		"<path>\t\t\tPath to Tracee binary",
	)

	stressCmd.Flags().String(
		"tracee-output",
		defaultTraceeOutput,
		"<format:path>\tTracee output format (e.g., json:/tmp/out.json, none to disable)",
	)

	stressCmd.Flags().Bool(
		"metrics",
		true,
		"\t\t\tEnable Tracee metrics collection (default: true for stress testing)",
	)

	stressCmd.Flags().Bool(
		"pprof",
		true,
		"\t\t\tEnable Tracee pprof profiling (default: true for stress testing)",
	)

	stressCmd.Flags().Bool(
		"pyroscope",
		false,
		"\t\t\tEnable Tracee pyroscope profiling (default: false)",
	)

	stressCmd.Flags().Bool(
		"keep-tracee",
		false,
		"\t\t\tKeep Tracee running after stress test (useful with --pyroscope)",
	)
