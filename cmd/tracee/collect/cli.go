...
func CLIAction() func(c *cli.Context) error {
	return func(c *cli.Context) error {
		...
		sockets, err := flags.ParseContainers(c.StringSlice("crs"))
		if err != nil {
			return err
		}
		cfg.Sockets = sockets

		cache, err := flags.ParseCache(c.StringSlice("cache"))
		if err != nil {
			return err
		}
		cfg.Cache = cache

		capture, err := flags.ParseCapture(c.StringSlice("capture"))
		if err != nil {
			return err
		}
		cfg.Capture = &capture

		filter, err := flags.ParseFilter(c.StringSlice("trace"))
		if err != nil {
			return err
		}
		cfg.Filter = &filter
		...
		output, printerConfig, err := flags.ParseOutput(c.StringSlice("output"))
		if err != nil {
			return err
		}
		...
		capsCfg, err := flags.ParseCapsConfig(c.StringSlice("caps"))
		if err != nil {
			return err
		}
